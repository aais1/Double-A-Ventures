...
const Product = () => {
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const { data } = await axios.post(
        `http://localhost:3000/products/review/${id}`,
        {
          username: "dummy",
          ProductId: id,
          ReviewText: review,
  ...
  const fetchReviews = async () => {
    const { data } = await axios.get(
      import.meta.env.VITE_SERVER_URL + "/products/review/" + id
    );
    setReviews(data);
  };
    ...
    <div className="w-screen mx-auto  md:w-[95vw] p-1 md:p-4">
      <div className="flex  flex-col md:flex-row gap-y-8 md:gap-y-0 gap-x-8 my-2 ">
        ...
        <div className="flex-1 lg:w-[60%]">
          ...
          <button
            className="bg-black text-md  text-white px-6 duration-150 active:scale-[0.90] md:px-14 py-2 rounded"
            onClick={() => {
      ...
      {user != null ? (
        <div className="w-[90vw] md:w-[75vw] mx-auto">
          <form onSubmit={handleSubmit}>
            ...
            <button
              className="w-[20%] md:w-[10%] rounded-tr-full rounded-br-full border border-blue-500 text-blue-500 font-semibold p-2 hover:bg-blue-500 hover:text-white duration-150 active:scale-95"
              type="submit"
            >
              Submit
            </button>
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 ">
          {reviews?.length === 0 ? (
            <p className="font-semibold text-lg p-2">No Reviews Yet</p>
          ) : (
            reviews?.map((review) => (
